...
;led N1 blinks if pi_end(RB1)
;led N2 lights if max or min duty(RB2) 
;led N3 blinks if timer4 overflow(RB3) 
...
; Declare usefull variables begining at the first GPR  adress of bank1 in ram
	...
	bsf 	OSCTUNE,6		; PLL enable
	;bcf 	OSCTUNE,6		; PLL enable
	
	; Configure ADC
	; 3.5V nominal value with vref+=4.1V -> 0x0DA7
	; 1 conversion = 15 TAD = 15Âµs
	; conversion f=30khz -> every 33Âµs
	movlb	0x0F
	movlw	b'00000000'
	...
	movlb	0x0F
	movlw	0x60 ;41	    ; with PLL
	movwf	PR4		    ; PR4 = 0x41 = 65
	movlw	b'01111000'
	movwf	T4CON
;	movlw	0x84 ;41	    ; without PLL
;	movwf	PR4		    ; PR4 = 0x41 = 65
;	movlw	b'00000000'
;	movwf	T4CON
